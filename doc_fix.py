...
class Window(Frame):
	...
	def init_window(self):
		...
		insertTopButton = Button(self, text="Bottom to Top",
			command = lambda: self.insert_tBox(beforeTextBox, afterTextBox.get("1.0", END)))

		deleteButton = Button(self, text="Clear Top", 
			command = lambda: self.clear_tBox(beforeTextBox))
		...
		fixSpecial = Button(self, text="Fix Special Char",
			command = lambda: self.convert_special(afterTextBox,beforeTextBox.get("1.0", END)))
		...
		createListButton = Button(self, text="Create list",
			command = lambda: self.create_list(afterTextBox,beforeTextBox.get("1.0", END)))
